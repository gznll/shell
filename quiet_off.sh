...
# Enable NTP client
echo -e "\n${YELLOW}[+] Enabling NTP client${NC}"
if sudo systemctl start systemd-timesyncd > /dev/null 2>&1; then
    echo -e "${GREEN}[*] Systemd NTP client enabled successfully.${NC}"
elif
   sudo systemctl start ntpsec > /dev/null 2>&1; then
    echo -e "${GREEN}[*] NTPSec daemon enabled succesfully.${NC}"
elif
   sudo systemctl start chronyd > /dev/null 2>&1; then
    echo -e "${GREEN}[*] Chronyd NTP daemon enabled succesfully.${NC}"
else
    echo -e "${RED}[!] Error when enabled the NTP client.${NC}"
    exit 1
fi
...
nmcli connection modify "Wired connection 1" ipv4.dhcp-send-hostname yes
...
if sudo systemctl restart NetworkManager; then
	echo -e "${GREEN}[*] NetworkManager restart successfully${NC}"
else
	echo -e "${RED}[!] Error NetworkManager restart.${NC}"
fi

echo -e "
{GREEN}[*] Reset script executed successfully${NC}"
hostname
exit 0
